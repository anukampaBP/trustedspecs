...
// ── Design tokens ─────────────────────────────────────────────────
const C = {
  bg:'#f2f1ed', surface:'#ffffff', border:'#e6e3db',
  text:'#17171a', muted:'#6e6e80', dim:'#b0aebf',
  brand:'#6d28d9', brandSoft:'#ede9fe', brandBorder:'#c4b5fd',
  green:'#15803d', greenBg:'#f0fdf4', greenBd:'#bbf7d0',
  amber:'#b45309', amberBg:'#fffbeb', amberBd:'#fde68a',
  red:'#be123c',   redBg:'#fff1f2',   redBd:'#fecdd3',
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  ::-webkit-scrollbar-thumb { background: #ccc9c0; border-radius: 4px; }
  ...
  .hover-lift:hover { transform: translateY(-3px) !important; box-shadow: 0 10px 32px rgba(0,0,0,.11) !important; }
  input[type=range] { -webkit-appearance:none; height:5px; border-radius:5px; outline:none; cursor:pointer; }
  input[type=range]::-webkit-slider-thumb { -webkit-appearance:none; width:20px; height:20px; border-radius:50%; background:${C.brand}; cursor:pointer; box-shadow:0 2px 8px rgba(109,40,217,.35); border:2px solid #fff; }
`
...
const QBar = ({ score }) => {
  ...
  return <div style={{ display:'flex', alignItems:'center', gap:8 }}>
    <div style={{ flex:1, height:5, background:'#ebe9e3', borderRadius:4, overflow:'hidden' }}>
...
function PhoneCard({ phone, navigate, onCompare, inCompare }) {
  return (
    <div className="hover-lift" onClick={() => navigate('detail', phone)}
      ...
      <div style={{ textAlign:'center', fontSize:38, margin:'4px 0 10px' }}>📱</div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
        {phone.launch_price > 0
          ? <span style={{ ...mono, fontSize:16, fontWeight:800, color:C.brand }}>₹{(phone.launch_price/1000).toFixed(0)}K</span>
...
function Nav({ page, navigate, compareCount }) {
  return (
    <nav style={{ background:C.surface, borderBottom:`1px solid ${C.border}`, padding:'0 24px',
      ...
      <div onClick={()=>navigate('home')} style={{ fontWeight:800, fontSize:17, color:C.brand,
      ...
      <div style={{ display:'flex', flex:1, justifyContent:'center', gap:2 }}>
        {[['home','Home'],['budget','Budget Finder'],['games','Games'],['news','News']].map(([p,l])=>(
          <button key={p} onClick={()=>navigate(p)} style={{
            ...
            fontFamily:"'Plus Jakarta Sans',sans-serif", transition:'color .15s' }}>{l}</button>
        ))}
      </div>
      <div style={{ display:'flex', gap:8, alignItems:'center' }}>
        {compareCount>0&&(
          <button onClick={()=>navigate('compare')} style={{
            ...
            fontSize:12, fontWeight:700, color:C.brand, cursor:'pointer',
        ...
        <button style={{ background:C.brand, color:'#fff', border:'none',
          ...
          cursor:'pointer', fontFamily:"'Plus Jakarta Sans',sans-serif" }}>🔔 Alerts</button>
      </div>
    </nav>
  )
}
...
function Footer() {
  return (
    <footer style={{ background:C.text, padding:'40px 24px 24px' }}>
...
function CompareBar({ list, onRemove, onClear, navigate }) {
  ...
  return (
    <div style={{ position:'fixed', bottom:20, left:'50%', transform:'translateX(-50%)',
      background:'#18181b', borderRadius:14, padding:'10px 16px',
      display:'flex', alignItems:'center', gap:10, zIndex:500,
      boxShadow:'0 8px 32px rgba(0,0,0,.25)', maxWidth:'90vw',
      ...
      <button onClick={()=>navigate('compare')} style={{ background:C.brand, color:'#fff',
        ...
        cursor:'pointer', fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Compare →</button>
      <button onClick={onClear} style={{ background:'none',
        border:'1px solid rgba(255,255,255,.15)', color:'rgba(255,255,255,.4)',
        borderRadius:6, padding:'4px 10px', fontSize:11, cursor:'pointer',
        fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Clear</button>
    </div>
  )
}
...
function Toast({ msg, onDone }) {
  ...
  return (
    <div style={{ position:'fixed', bottom:80, right:20, background:C.text,
      color:'#fff', borderRadius:10, padding:'10px 18px', fontSize:13, fontWeight:500,
      boxShadow:C.shadowLg, animation:'toastIn .3s ease', zIndex:600 }}>{msg}</div>
  )
}
...
function HomePage({ navigate, addToCompare, compareList }) {
  ...
  return (
    <div style={{ animation:'fadeUp .4s ease' }}>
      ...
      {/* Hero */}
      <section style={{ background:`radial-gradient(ellipse 90% 50% at 50% -5%, ${C.brandSoft}, ${C.bg} 70%)`,
        padding:'64px 24px 52px', textAlign:'center' }}>
        <h1 style={{ fontSize:'clamp(26px,5vw,50px)', fontWeight:800, color:C.text,
          letterSpacing:-1.5, lineHeight:1.15, marginBottom:14 }}>
          The phone comparison site<br/>
          <span style={{ color:C.brand }}>that actually tells the truth.</span>
        </h1>
          ...
          <div style={{ display:'flex' }}>
            <input value={query} onChange={e=>setQuery(e.target.value)} onFocus={()=>setFocused(true)}
              ...
              style={{ flex:1, padding:'14px 18px', fontSize:13, border:`1.5px solid ${C.border}`,
                ...
                fontFamily:"'Plus Jakarta Sans',sans-serif", background:C.surface, color:C.text }}
          ...
          <div style={{ display:'flex', gap:28, justifyContent:'center', marginTop:32, flexWrap:'wrap' }}>
            {[[stats.phones,'Phones reviewed'],[stats.brands,'Brands covered'],['Live','Indian pricing'],['Free','No account needed']].map(([n,l])=>(
              <div key={l} style={{ textAlign:'center' }}>
                <div style={{ ...mono, fontSize:20, fontWeight:800, color:C.brand }}>{n}</div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:40, alignItems:'center', flexWrap:'wrap' }}>
            <div>
              ...
              <button onClick={()=>navigate('budget')} style={{ background:C.brand, color:'#fff',
                ...
                cursor:'pointer', fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Open Budget Finder →</button>
            </div>
          ...
          <div style={{ maxWidth:1060, margin:'0 auto' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:24 }}>
              ...
              <button onClick={()=>navigate('budget')} style={{ background:'none', border:`1px solid ${C.border}`,
                borderRadius:8, padding:'7px 16px', fontSize:12, fontWeight:600, color:C.brand,
                cursor:'pointer', fontFamily:"'Plus Jakarta Sans',sans-serif" }}>See all →</button>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(180px,1fr))', gap:12 }}>
              {stats.trending.map(p=>(
                <PhoneCard key={p.id} phone={p} navigate={navigate}
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
          {CASES.map(([k,l])=>(
            <button key={k} onClick={()=>setUseCase(k)} style={{
              ...
              fontFamily:"'Plus Jakarta Sans',sans-serif", transition:'all .15s' }}>{l}</button>
          ))}
        </div>
      </div>
      {loading ? <Spinner/> : (
  ...
  if (!compareList.length) return (
    <div style={{ textAlign:'center', padding:80, color:C.muted }}>
      ...
      <button onClick={()=>navigate('home')} style={{ background:C.brand, color:'#fff', border:'none',
        ...
        fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Browse phones →</button>
    </div>
  )
            ...
            <>
              ...
              {mystery.answer ? (
                ...
                <button onClick={revealMore} style={{ width:'100%', background:C.brand, color:'#fff', border:'none',
                  ...
                  fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Reveal clue {revealed+1}</button>
              ) : (
                <div style={{ display:'flex', gap:6 }}>
                  <input value={guess} onChange={e=>setGuess(e.target.value)} placeholder="Your guess…"
                    style={{ flex:1, border:`1px solid ${C.border}`, borderRadius:8, padding:'8px 10px',
                      fontSize:12, outline:'none', fontFamily:"'Plus Jakarta Sans',sans-serif" }}/>
                  <button onClick={revealMore} style={{ background:C.brand, color:'#fff', border:'none',
                    ...
                    fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Go</button>
                </div>
              )}
            </>
          ) : <Spinner/>}
          ...
          <div style={{ background:C.bg, borderRadius:10, padding:10, marginBottom:12 }}>
            ...
            {[['50MP f/1.8 main + 12MP ultra + 10MP tele',false],['4500mAh battery with 45W charging',true],['IP68 dust & water resistance',false]].map(([f,bluff],i)=>(
              <button key={i} style={{ width:'100%', display:'block', marginBottom:5, padding:'8px 10px',
                ...
                fontSize:11, color:C.text, cursor:'pointer', fontFamily:"'Plus Jakarta Sans',sans-serif", textAlign:'left',
                transition:'all .15s' }}
